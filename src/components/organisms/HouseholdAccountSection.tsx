...
export const HouseholdAccountSection: React.FC = ({onPressButton, items, expenses}) => {
  const elementButton = (expense) => {
    return (
  ...
  itemsArray.unshift('');
  const daysFlexArray = [...Array(itemsArray.length)].map((_) => 1);
  const [tableHead] = useState(itemsArray);

  const daysArray = [...Array(7)].map((_, i) => dayjs().add(i, 'days').format('MM/DD'));
  const daysHeightArray = [...Array(daysArray.length)].map((_) => 28);
  const [tableTitle] = useState(daysArray);

  const expensesArray = (expenses && expenses.length !== 0) ? expenses.map(expenseOneday => expenseOneday.map((expense) => elementButton(expense))) : [];
      ...
      <Table borderStyle={{borderWidth: 1}}>
        ...
        <ScrollView>
          <TableWrapper borderStyle={{borderWidth: 1}} style={styles.wrapper}>
            <Col data={tableTitle} style={styles.title} heightArr={daysHeightArray} textStyle={styles.text}/>
