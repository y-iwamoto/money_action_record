...
function getFundamentalItems(todayDiff: number, items: Item[], expenses: Expense[][] | []) {
  ...
  return { labels, data, datasets, legend };
}

function getDataAccumulate(data: number[][], items: Item[]) {
...
export function UseSetChartDataHook(
  ...
  const { labels, data, datasets, legend } = getFundamentalItems(todayDiff, items, expenses);
  setChartData({ labels: labels, datasets: datasets, legend: legend });
  const dataAccumulateSets = getDataAccumulate(data, items);
